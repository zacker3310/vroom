...
line and no timer. And beating all eighty unlocks the Victory Parade: a rainbow-road
finale with fireworks, every buddy you've found cheering from the roadside, and a
champion's jackpot at the trophy arch.
...
- **Save codes**: the same panel shows a scan-to-open QR — point another device's camera
  at it and the game opens with the save ready to import. Copy/paste buttons carry a
  full save code, and opening `https://zacker3310.github.io/vroom/#save=<code>` imports
  one directly.
...
- The QR code is produced by a hand-rolled encoder (byte mode, ECC level L, versions
  1 to 10 with multi-block Reed-Solomon interleaving over GF(256)) verified byte-exact
  against a real decoder.
- `tests/` holds ten Playwright suites with 161 checks covering the economy, physics,
