...
## Album & buddies Doubt (T4.4)

Audited the uncommitted T4.4 diff (sticker album + buddies + finish photos, index.html + tests/album-check.cjs vs 05817e0, +262/-16). All manifest claims verified against the diff: BUDDY_ORDER/BUDDY_SVG (6 peeking critters), buddy rendered behind the body at roof (-55,+4) with nested-g .buddyBob (CSS anim can't clobber the translate), state.buddy validated vs owned on load, progress.owned.buddy/badges/photos with loadState validation + 6-photo cap, capsule buddy prize w2 (unfound uniform draw -> own + auto-equip + buddyPop + carWrap rebuild; all-found -> 5-star consolation, PRIZE_TABLE untouched), BADGES x6 with once-only award() + wordless z-70 toast on its own timer, hooks at ramp launch / finishLevel (clean/S/world + photo unshift) / priceTag / runGag, sfx.squeak, album scene (tap-to-equip w/ equipped ring + wiggle-deny, brightness(0) badge silhouettes, 6 tilted polaroids), albumBtn/albumHomeBtn/albumWallet wired into renderWallets, squeak on preview/carWrap taps. Prize-roll stubs in free-check/worlds-check correctly retuned for the 13-weight table (0.8*13=10.4 gag [8,11); 0.57*13=7.41 golden [7,8); 0.9*13=11.7 buddy [11,13)).

Findings and probes (doubt-t44.cjs, scratchpad; shots d-buddy-grid*.png, d-buddy-extras.png, d-rainbow-photos.png):

- **Buddy anchors across all 15 bodies (screenshot grid, kitty/pup/monkey each):** every body reads correctly — head above the roofline, paws tucked behind the silhouette; nothing floats detached or sinks. Worst suspects clear: race reads as sitting in the cockpit, ufo peeks from behind the dome, limo/rocket/train/royal/dragon all charming. No per-body buddyAt override needed.
- **Buddy + roof extras (dump/tractor/police x horn/beacon/partyhat):** horn lands at the buddy's mouth (reads as the buddy blowing it), partyhat sits ON the buddy's head, beacon rides beside the ears — accidental delight, not collision. No change.
- **MED (fixed): current damage bled onto keepsake polaroids.** vehicleSVG bakes `progress.damage` overlay visibility into every render, so a banged-up car put scuffs/cracks/smoke on all six album photos — including photos of clean finishes. Added `.photoCar .dmgScuff/.dmgCrack/.dmgSmoke { display: none !important }` so keepsakes stay pristine (index.html:230; !important needed to beat the inline display).
- **MED (fixed): mud splash vanished when a buddy capsule rebuilt carWrap mid-run.** The rebuild refreshed wheelEls/mudEl but re-rendered mudSpots at opacity 0, silently washing the car mid-puddle-run. Now captures muddy-state before the innerHTML swap and re-applies it (index.html:2815). Probe confirms beams (night), damage overlays, wobble class, and 2 wheel refs all survive the rebuild — mud now does too.
- **LOW (fixed): loadState photo filter didn't require p.b.extras.** A tampered/truncated save with a photo lacking extras would throw in extrasSVG and blank the album. Added `p.b.extras` to the existing validation chain (index.html:770) — same spirit as the body/wheels/color checks beside it.
- **Pre-existing test flake (fixed): free-check "props spawn ahead" required ahead>=3** in a 1600px window with 280-540px spawn spacing + a 500px post-ramp skip — legitimately produces 2 (~1-in-5 runs failed, reproduced pre-fix twice with identical pos=1484). Relaxed to >=2 with a comment (tests/free-check.cjs:97); 3 consecutive green runs after.
- Rainbow photos: 6 rainbow polaroids render 6 linearGradients with unique ids (rg1-rg6), each svg carrying its own url(#id) paint — svgUid is global and monotonic, no collision with the garage preview. PASS.
- Toast: computed pointer-events none over the road HUD; toastTimer deliberately outside gagTimers is safe — a scene switch mid-toast still gets the .show removed at 1.9s (probed, no stuck toast). PASS.
- Save eras: v1-only build, pre-album v2 (wallet/owned/levels/damage/upgrades, no buddy/badges/photos keys), and fresh save all load with defaults holding (empty arrays, buddy null), album renders 6+6+6 slots, zero console errors. PASS.
- Equip flow: album equip shows the buddy in the garage preview immediately (showGarage -> renderPreview); dice shuffle (8 spins) and priceTag purchases never touch state.buddy — buddy still riding after both. PASS.
- Economy math: buddy weight 2/13 = 15.4% per capsule; each hit draws an unfound buddy uniformly, so exactly 6 hits complete the set -> expected 6*(13/2) = 39 capsules. Levels carry 1 capsule (2-8) then 2 (9-80): 39 capsules = every capsule through ~level 24, or ~level 43 at 50% pickup — all six well inside one 80-level playthrough, consolation path only after that. No tuning needed.

Verification after fixes: verify 35/35, polish-check 14/14, damage-check 20/20, free-check 14/14 (x3 runs), feel-check 12/12, worlds-check 18/18, album-check 11/11 = **124/124 PASS**, plus doubt-t44 16/16, no console errors.

## Profiles & save codes Doubt (T6)

Audited the uncommitted T6 diff (profiles + QR save codes, index.html + tests/profile-check.cjs + verify.cjs vs 3aa2f3e, +379/-5). All manifest claims verified against the diff: vroom.meta {active, avatars[3]} with AVATAR_IDS validation + clamping, vroom.v2 -> vroom.v2.p0 migration (v1 migration correctly gated to profile 0), per-profile saveKey()/save()/loadState(), switchProfile persist + reload, garage #profileBtn (avatar face, 24/24; upgrades to top 136, dice to right 96 top 300), #profileOverlay 3 slots + #saveTools (QR canvas, copy/paste clipboard), packCompact/unpackCompact (VROOM1. 51B bit-packed vs FROZEN CODE_* orders), exportFullCode (VROOM2. gzip / VROOM3. fallback), importSaveCode -> #importConfirm gate, #save= hash import, hand-rolled QR encoder (byte mode, ECC L, v1-5, RS GF256, mask 0, BCH format info) verified byte-exact against jsQR.

Findings and probes (audit.cjs, scratchpad; shot audit-garage-damaged.png):

- **HIGH (fixed): a mangled %-escape in #save= bricked the boot.** `decodeURIComponent('#save=%ZZ'.slice(6))` throws URIError at top level, aborting the rest of the script — every listener registered after the hash-import block (albumHomeBtn, goBtn, homeBtn, mapHomeBtn, celebrateHomeBtn, and everything below) never attached, and the poisoned hash survived reloads since replaceState never ran. Wrapped the decode in try/catch, moved replaceState ahead of the import, and only import a non-empty code (index.html:2591). Regression check added to profile-check.cjs (malformed #save= boots clean, hash cleared).
- **MED (fixed): truncated VROOM1 codes decoded to near-empty saves and raised the confirm.** unpackCompact's bit reader runs past the end of a short payload reading zeros (undefined>>x coerces to 0), so a 1-byte code with a valid version byte produced a structurally valid empty save — a corrupted scan could present a confirm that, if accepted, wipes progress to nothing. Added an underflow check after the reads: `if (pos > bytes.length * 8) return null` (index.html:2384) — length-agnostic, so appending frozen fields later still works. Truncated-code rejection added to the profile-check garbage test.
- QR robustness sweep: fresh empty save, maxed save (wallet 65535, all owned, 6 buddies + 6 badges, 80 levels rating-3 tier-S, upgrades 3/3/3, damage 9, quiet, monkey riding) — both 75 chars -> v4 (33 modules), jsQR byte-exact, maxed round-trips every field. Synthetic payload sweep at every version boundary (1/16/17/18/31/32/33/52/53/54/77/78/79/105/106 bytes) all decode; 107 correctly returns null. Decodes at the actual rendered geometry (140px canvas, and v5's floor(164/37)=4px modules). Encoder is sound across its whole envelope. PASS.
- Tamper sweep: 14 hostile codes (8 random 51B, truncated 1B/10B, oversized 200B, empty payload, corrupt gzip VROOM2, corrupt JSON VROOM3) — zero crashes; truncated + corrupt reject post-fix. Random-but-valid 51B codes confirm (structurally legitimate saves) and loadState fully sanitizes on reload: wallet/current/damage/upgrades clamped, owned/levels/badges filtered to known ids. Crafted VROOM3 with wallet -5000 + junk fields passes the Number.isFinite gate but lands as pristine defaults after reload; null owned and string wallet reject at the gate. The two-gate design holds. PASS.
- Wallet overflow: 9,999,999 stars packs as 65535 (clamped, no bit bleed into current). PASS.
- Profile edges: 3 profiles get 3 distinct avatars (find-first-unused; the "pup" fallback is unreachable — 6 avatars, 3 slots). profileBtn renders the switched-to avatar after reload. profileBtn lives inside #garage so it's unreachable mid-run (offsetParent null on the road) — switchProfile's save() only ever fires in garage context, unbanked runStars are never at risk. Tampered meta (active 7, all-null avatars) clamps to active 2, restores pup on slot 0, boots with the fallback face on the button; playing a null-avatar slot works (own save key). PASS, no change.
- Import targeting: pendingImport applies to whoever is active at Yes-time, and it cannot cross profiles — switchProfile reloads the page, discarding any pending import. Import-lands-on-active is the sane, intended behavior. Report only.
- Clipboard denied: copy and paste both catch and play deny, no crash (probed with clipboard mocked to reject). Note a cosmetic double-deny on pasting an invalid code (walletDeny inside importSaveCode + sfx.deny in the paste handler fire near-simultaneously) — inaudible in practice, LOW, left alone.
- Codes are pure b64url: compact + full both match /^VROOM[123]\.[A-Za-z0-9_-]*$/ — no raw '+' to be eaten by URL decoding. Empty #save= and double-hash (#save=abc#def) boot clean, no confirm, hash cleared. PASS.
- Layout: pairwise bounding-box intersection across profileBtn/upgrades/quietBtn/albumBtn/garageWallet/diceBtn/repairBtn/controls/garageSign on a damaged save (repair wrench visible) — zero overlaps; upgrades end y452 vs controls y456, dice y300-396 vs repair y150-270. Screenshot reads clean. PASS.
- LOW (report only): compact codes assign tier "C" to any level that had a rating but no tier (TIER_RANK.C is 0, indistinguishable from "no tier" in 2 bits) — a rating-only level from an old save gains a C medal after a QR round-trip. Cosmetic fidelity loss on the lossy codec's already-lossy path; not worth a format bump.

Verification after fixes: verify 35/35, polish-check 14/14, damage-check 20/20, free-check 14/14, feel-check 12/12, worlds-check 18/18, album-check 11/11, profile-check 13/13 (grew 12 -> 13 with the malformed-hash regression check) = **137/137 PASS**, plus audit.cjs 19/19, no console errors.

## Toybox overhaul Doubt (T7)

Audited the uncommitted index.html + tests/verify.cjs delta vs 1849948 (Toybox design-system layer, tray/journey/resultsCard restructure, decal + medal work). All eight suites re-run first: verify 35/35, polish 14/14, damage 20/20, free 14/14, feel 12/12, worlds 18/18, album 11/11, profile 13/13 = **137/137 PASS**, no console errors. Then a 20-screenshot visual pass (fresh/rich/locked/damaged garage, map w1/w4/w8, album empty/rich, profile overlay, importConfirm, roads w1/w4/w5/w8, celebrate day+night, free drive, portrait rotate, mid-iris) plus programmatic geometry/ink/selector scans. **Verdict: PASS — no HIGH, no MEDIUM. No code changes made.**

- Scene-by-scene visual bar: garage trays, wood workbench, cream chrome, map journey path, album paper, profile panel, importConfirm, celebrate resultsCard, rotate overlay all read cohesive and warm. HUD chips legible on snow-white w5 road and w8 night sky (cream fill + tan border + flat shadow carry the contrast). PASS.
- Geometry sweep (bounding-box): trays fit the 1200 stage with room (goCol right edge 965/1024 viewport px, zero pairwise overlaps); map nodes never intersect worldTabs (tab bottom 261 vs node top 374), freeBtn, or stage edges in w1/w4/w8; resultsCard doesn't touch celebrateWallet or celebrateRow and stays inside the stage; homeBtn confirmed display:none while celebrating; celebrate has exactly 3 children. PASS.
- Design-layer dead rules (authored pre-tray/journey/resultsCard): **6 dead-in-practice selectors, all harmless, left in place** — `.worldTab span` (WORLD_ICONS covers all 8 worlds, span is fallback-only), `#celebrate.active > :nth-child(4)` and `:nth-child(5)` (celebrate now has 3 children), `.chip [fill="#023047"]` + `.chip [stroke="#023047"]` (no navy attrs remain inside chips; the photo-caption flag lives in a .bigBtn slot and is covered there), and the `.lvlBtn .lockDot` half of one selector (journey nodes use lockIcon innerHTML, lockDots only exist on swatches/extras). No live conflicts found: no #mapGrid grid rules in the layer, celebrate flex gap is compatible with the card, `.lvlBtn.locked:not(.far)` (0,3,0) correctly beats the layer's `.lvlBtn` radius so padlock nodes stay circular.
- #freeBtn: the layer's `border-radius: var(--v-r-lg)` re-squares the structural circle into a 130px rounded square. Screenshot-checked in w1/w4/w8 — reads as the same chunky-square family as worldTabs/lvlBtns, steering-wheel icon centered. Kept as-is (endorsed in the payload).
- Entrance staggers: renderMap rebuilds tabs+nodes on every tab tap so popIn replays — reads as intended AC-style juice, no measurement risk beyond the patched verify.cjs touch audit (grep: verify.cjs is the only getBoundingClientRect user; its pedal boundingBoxes are road-scene, no .enter there; other suites tap via dispatchEvent, and Playwright's click actionability waits for animation stability). Max garage stagger is 385ms delay + 300ms anim = 685ms < the 1100ms settle. PASS.
- .pressable:active vs existing transforms: `.pressable:active` (source order after `.swatch.selected`, equal 0,2,0 specificity) wins during press → uniform squash, and selected scale(1.1) restores on release (computed at rest: matrix(1.1,...)); worldTab.sel uses outline (no transform channel); bigBtn+pressable combos resolve to the layer's later `.bigBtn:active` squash. No conflict. Selected-swatch rendered size 71.3px, unselected 64.8px ≥ 64 SPEC floor. PASS.
- Damage decals: screenshot dragon d1/d3/d5, limo d1/d5, tractor d5, dump d5 — bandage visible at every rear anchor (limo trunk, dragon tail, tractor fender), smoke visibly anchored (tractor: wraps the exhaust stack; limo: sits on the chauffeur cap; dragon: at the head/neck). `Math.min(288, roof[0]+60)-288` is sane for tractor (roof 142 → shift -86, puffs land at x≈202 on the stack) and clamps to 0 for long bodies (dump roof 245). Programmatic check: smoke + bandage inside the preview SVG bounds for all four bodies. PASS.
- medalSVG S-scallop: legible at all three sizes — celebrate time chip (68px), map tierBadge (~28px crop-checked: orange burst reads as a distinct silhouette vs A/B circles), album photo captions. PASS.
- Iris: #3a2c1e mid-wipe screenshot reads as intentional warm chocolate against the #2f241a letterbox; settles at opacity 0 / pointer-events none, follow-up frame clean. No stuck frames. PASS.
- Icon ink: computed-style scan over every button/chip in all scenes for surviving cold paints (rgb(2,48,71) / rgb(108,117,125)) — zero navy chrome anywhere; only hit is a gray stroke inside buddy-slot artwork (character art, not chrome). Muted-speaker + HUD flagpole grays recolor via the attribute selectors when live. PASS.
- LOW (report only): journey padlock node renders at 61.4px (72 × .853 stage scale), a hair under the 64px SPEC floor, and .far dots at 37.5px — both are deny-feedback-only targets, not primary actions; verify.cjs deliberately audits garage only. Locked chrome (white lock on putty at reduced opacity) is low-contrast by design and still discernible in screenshots. Left alone.
